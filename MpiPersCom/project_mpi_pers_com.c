...
int main (int argc, char *argv[]) {
    ...
    if (tasks - 1 < taskid) {}    // EXTRA TASKS THAT DON'T DO NOTHING
        ...
        // DO MPI_Send_init X 4 FOR Isend
            MPI_Send_init(&(u1[(num_y+2) * 1 + 1]), 1, up_down_rows, row_column[taskid].up, 0, MPI_COMM_WORLD, &(row_column[taskid].Isend_up));
            
            MPI_Send_init(&(u1[(num_y+2) * (num_x + 0) + 1]), 1, up_down_rows, row_column[taskid].down, 0, MPI_COMM_WORLD, &(row_column[taskid].Isend_down));
                        
            MPI_Send_init(&(u1[(num_y+2) * 1 + 1]), 1, left_right_columns, row_column[taskid].left, 0, MPI_COMM_WORLD, &(row_column[taskid].Isend_left));
                        
            MPI_Send_init(&(u1[(num_y+2) * 1 + (num_y + 0)]), 1, left_right_columns, row_column[taskid].right, 0, MPI_COMM_WORLD, &(row_column[taskid].Isend_right));

        // DO MPI_Recv_init X 4 FOR Irecv
            MPI_Recv_init(&(u1[(num_y+2) * 0 + 1]), 1, up_down_rows, row_column[taskid].up, 0, MPI_COMM_WORLD, &row_column[taskid].Irecv_up);
                    
            MPI_Recv_init(&(u1[(num_y+2) * (num_x + 1) + 1]), 1, up_down_rows, row_column[taskid].down, 0, MPI_COMM_WORLD, &(row_column[taskid].Irecv_down));
                    
            MPI_Recv_init(&(u1[(num_y+2) * 1 + 0]), 1, left_right_columns, row_column[taskid].left, 0, MPI_COMM_WORLD, &(row_column[taskid].Irecv_left));
                    
            MPI_Recv_init(&(u1[(num_y+2) * 1 + (num_y + 1)]), 1, left_right_columns, row_column[taskid].right, 0, MPI_COMM_WORLD, &(row_column[taskid].Irecv_right));

        for (it = 1; it <= STEPS; it++) {
            // DO MPI_Start X 4 FOR Isend
                MPI_Start(&(row_column[taskid].Isend_up));

                MPI_Start(&(row_column[taskid].Isend_down));
                
                MPI_Start(&(row_column[taskid].Isend_left));
                
                MPI_Start(&(row_column[taskid].Isend_right));

            // DO MPI_Start X 4 FOR Irecv

                MPI_Start(&(row_column[taskid].Irecv_up));

                MPI_Start(&(row_column[taskid].Irecv_down));
                
                MPI_Start(&(row_column[taskid].Irecv_left));
                
                MPI_Start(&(row_column[taskid].Irecv_right));
