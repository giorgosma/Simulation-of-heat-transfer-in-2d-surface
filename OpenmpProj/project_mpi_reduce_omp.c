...
int main (int argc, char *argv[]) {
    ...
    int NXPROB = atoi(argv[3]);
    int NYPROB = atoi(argv[4]);
    ...
    change_column = 0;          // GLOBAL VARIABLE FOR CHANGED DATA
    int result_column = 0;      // VARIABLE FOR CHECKING IF OTHER TASKS HAS CHANGED DATA
    ...
    startTime = MPI_Wtime();
    if (tasks - 1 < taskid) {           // EXTRA TASKS THAT ONLY CALL ALLREDUCE FOR NUMBER OF STEPS
        for (it = 1; it <= STEPS; it++)
            MPI_Allreduce(&change_column, &result_column, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
    }    
    else {
        for (it = 1; it <= STEPS; it++) {
            // DO Isend X 4
            ...
            // not sure if pragma needed outside of update
            ...
            MPI_Allreduce(&change_column, &result_column, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
            if (result_column != 0) {   // DATA HAVE CHANGED
                change_column = 0;
                result_column = 0;
                // printf("%d Changed\n", taskid);
            }
            else {                      // DATA HAVE NOT CHANGED
    ...
    if (taskid == 0) {          // MASTER TASK TAKES ALL TIMES AND MAKE AVG, MIN AND MAX TIME
        ...
        printf("--TASK: %d\tCOUNTING AVG: %f\tCOUNTING MAX: %f\tCOUNTING MIN: %f\n", taskid, counting_time_avg, counting_time_max, counting_time_min);
    }
    else
    {
    ...
    for (ix = start; ix <= end; ix++) {
        for (iy = y_st; iy <= y_end; iy++) {
            *(u2+ix*numy+iy) = *(u1+ix*numy+iy)  +
            ...
            {
                if (memcmp((u2+ix*numy+iy), (u1+ix*numy+iy), sizeof(float)) != 0)
                    change_column++;
            }
